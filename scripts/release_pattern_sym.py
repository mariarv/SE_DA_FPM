...
firing_types = [ 'random','synchronous' ]
firing_types_burst = [ 'synchronous','random' ]
...
def generate_tonic_spike_trains(num_neurons, total_time, dt, tonic_rate, tonic_interval_noise, firing_type, min_isi_time_points):
    ...
    mean_firing_rate = 3  # Mean firing rate in Hz
    firing_rates = np.random.normal(mean_firing_rate, 2, num_neurons)

    # Ensure that no firing rates are greateer than 0.1HZ 
    firing_rates = np.clip(firing_rates, 0.1, None)
    print(np.mean(firing_rates))
    spike_trains = []
    jitter=0.05            
    ...
    for i in range(num_neurons):
        ...
        if firing_type == "synchronous":
            ...
            spike_train = np.zeros(time_points)
            for t in range(0, time_points, mean_interval_between_spikes):
                # Add jitter to the spike time
                jitter = np.random.randint(-5, 5 + 1)
                spike_time = t + jitter
                if 0 <= spike_time < time_points:
                    spike_train[spike_time] = 1

            spike_trains.append(spike_train)
    
        elif firing_type == "random_uni":
            # Uniform firing: Each neuron generates spikes at regular intervals with some jitter
            spike_train = np.zeros(time_points)
            
            # Calculate the interval between spikes based on the tonic rate
            avg_spike_interval_time_points = mean_interval_between_spikes

            # Start generating spikes at regular intervals, but add a bit of random jitter
            spike_times = []
            current_time_point = 0
            
            while current_time_point < time_points:
                jitter = np.random.uniform(-0.5, 0.5) * avg_spike_interval_time_points  # Jitter can be adjusted
                next_spike_time = current_time_point + avg_spike_interval_time_points + jitter
                next_spike_time = int(np.clip(next_spike_time, current_time_point + min_isi_time_points, time_points - 1))
                
                spike_times.append(next_spike_time)
                current_time_point = next_spike_time
...
# Parameters
num_neurons_tonic = 98
num_neurons_bursting = 2
num_neurons_bursting_rand = 15
...
for tonic_firing_type in firing_types:
    for bursting_firing_type in firing_types_burst:
